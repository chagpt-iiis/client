...
const
	...
	convRight = <BufferTransform[]>[
		...
		(bin: binaryString) => Uint8Array.from(bin, ch => ch.charCodeAt(0)),
		(buf: Uint8Array): hexString => Array.from(buf, byte => byte.toString(16).padStart(2, '0')).join(''),
	...
	convLeft = <BufferTransform[]>[
		...
		(buf: Uint8Array): binaryString => cc2str(buf),
		(hex: hexString) => Uint8Array.from({ length: hex.length / 2 }, (_, id) => parseInt(hex.substring(id * 2, (id + 1) * 2), 16)),
